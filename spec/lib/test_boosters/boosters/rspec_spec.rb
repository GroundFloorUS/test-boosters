...
describe TestBoosters::Boosters::Rspec do
  ...
  describe "#file_pattern" do
    before { ENV["RSPEC_TEST_FILE_PATTERN"] = "feature/features/**/*_spec.rb" }

    context "when the RSPEC_TEST_FILE_PATTERN environment variable is set" do
      it "returns its values" do
        expect(booster.file_pattern).to eq("feature/features/**/*_spec.rb")
      end
    end

    context "when the RSPEC_TEST_FILE_PATTERN environment variable is not set" do
      before { ENV.delete("RSPEC_TEST_FILE_PATTERN") }
